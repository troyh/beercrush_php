...
# Make sure /etc/init.d/spread is executable
sudo chmod ug+x /etc/init.d/spread
# Make sure /var/run/spread exists
if [ ! -d /var/run/spread ]; then
	sudo mkdir /var/run/spread;
fi

# TODO: make directories (/var/run/spread)
# TODO: make sure /etc/hosts doesn't list host as 127.0.0.1
