...
if ! sed -e "s/MYIPADDRESS/$MYIP/" conf/supervisord/appserver.conf | diff -u - /etc/supervisor/conf.d/BeerCrush-app.conf > /dev/null; then
	echo "**************************************************";
	echo "ERROR: Supervisor config (/etc/supervisor/conf.d/BeerCrush-app.conf) is incorrect:";
	echo "**************************************************";
	sed -e "s/MYIPADDRESS/$MYIP/" conf/supervisord/appserver.conf | diff -u - /etc/supervisor/conf.d/BeerCrush-app.conf;
	exit;
fi
